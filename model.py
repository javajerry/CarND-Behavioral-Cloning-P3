...
flags.DEFINE_string('log', 'data/driving_log.csv', 'Driving log')
flags.DEFINE_string('log2', 'CAR2/driving_log.csv', 'Curve Driving log')
flags.DEFINE_string('log3', 'CAR3/driving_log.csv', 'Reverse Curve Driving log')
flags.DEFINE_string('log4', 'CAR4/driving_log.csv', 'Curve Driving log')
flags.DEFINE_string('log5', 'CAR5/driving_log.csv', 'Curve Driving log - Hardturn')
flags.DEFINE_string('log6', 'CAR6/driving_log.csv', 'Curve Driving log')
...
flags.DEFINE_string('image_path2','CAR2/IMG/', "Image path")
flags.DEFINE_string('image_path3','CAR3/IMG/', "Image path")
flags.DEFINE_string('image_path4','CAR4/IMG/', "Image path")
flags.DEFINE_string('image_path5','CAR5/IMG/', "Image path")
flags.DEFINE_string('image_path6','CAR6/IMG/', "Image path")

#steering correction
flags.DEFINE_float('correction', 0.15, "Angle offset for Steering") 
#image shape
...
def read_log():
	...
	#hard turns bridge
	#with open(FLAGS.log2) as csvfile:
	#	print('read csv file - ', FLAGS.log2)
	#	for line in reader:
	#		samples.append(line)
	#		steer_angles.append(float(line[3]))

	#reverse bridge		
	#with open(FLAGS.log3) as csvfile:
	#	print('read csv file - ', FLAGS.log3)
	#	reader = csv.reader(csvfile)
	#	for line in reader:
	#		samples.append(line)
	#		steer_angles.append(float(line[3]))

	#more training data for first curver after the bridge
	with open(FLAGS.log4) as csvfile:
		print('read csv file - ', FLAGS.log4)
		reader = csv.reader(csvfile)
		for line in reader:
			samples.append(line)
			steer_angles.append(float(line[3]))

	#hard turns for first curve after the bridge
	with open(FLAGS.log5) as csvfile:
		print('read csv file - ', FLAGS.log5)
		reader = csv.reader(csvfile)
		for line in reader:
			samples.append(line)
			steer_angles.append(float(line[3]))

	#hard turns for second curve after the bridge
	with open(FLAGS.log6) as csvfile:
		print('read csv file - ', FLAGS.log6)
		reader = csv.reader(csvfile)
		for line in reader:
			samples.append(line)
			steer_angles.append(float(line[3]))
...
'''
def transform_image(x, steering, rot_angle=0.05, shift=0.09):
	valPixels = 0.4
	...
	while 1: # Loop forever so the generator never terminates
		...
		for offset in range(0, num_samples, batch_size):
			...
			for batch_sample in batch_samples :
				...
				if center_source_path.find("CAR2") > -1:
					#print(filename)
					current_path = FLAGS.image_path2 + filename
				elif center_source_path.find("CAR3") > -1:
					#print(filename)
					current_path = FLAGS.image_path3 + filename
				elif center_source_path.find("CAR4") > -1:
					#print(filename)
					current_path = FLAGS.image_path4 + filename
				elif center_source_path.find("CAR5") > -1:
					#print(filename)
					current_path = FLAGS.image_path5 + filename
				elif center_source_path.find("CAR6") > -1:
					#print(filename)
					current_path = FLAGS.image_path6 + filename
				else:
					#print(filename)
					current_path = FLAGS.image_path + filename
				image = cv2.imread(current_path)
				...
				left_source_path = batch_sample[1]
				filename = left_source_path.split('\\')[-1]
				if left_source_path.find("CAR2") > -1:
					current_path = FLAGS.image_path2 + filename
				elif left_source_path.find("CAR3") > -1:
					current_path = FLAGS.image_path3 + filename
				elif left_source_path.find("CAR4") > -1:
					current_path = FLAGS.image_path4 + filename
				elif left_source_path.find("CAR5") > -1:
					current_path = FLAGS.image_path5 + filename
				elif left_source_path.find("CAR6") > -1:
					current_path = FLAGS.image_path6 + filename
				else:
					current_path = FLAGS.image_path + filename
				image = cv2.imread(current_path)
				...
				if random.random() > 0.5:
					#image = cv2.imread(current_path)
					image, angle_left = transform_image(image, angle_left)
					images.append(image)
					angles.append(angle_left)	
				...
				right_source_path = batch_sample[2]
				filename = right_source_path.split('\\')[-1]
				if right_source_path.find("CAR2") > -1:
					current_path = FLAGS.image_path2 + filename
				elif right_source_path.find("CAR3") > -1:
					current_path = FLAGS.image_path3 + filename
				elif right_source_path.find("CAR4") > -1:
					current_path = FLAGS.image_path4 + filename
				elif right_source_path.find("CAR5") > -1:
					current_path = FLAGS.image_path5 + filename
				elif right_source_path.find("CAR6") > -1:
					current_path = FLAGS.image_path6 + filename
				else:
					current_path = FLAGS.image_path + filename
				image = cv2.imread(current_path)
				...
				if random.random() > 0.5:
					#image = cv2.imread(current_path)
					image, angle_right = transform_image(image, angle_right)
					images.append(image)
					angles.append(angle_right)	
